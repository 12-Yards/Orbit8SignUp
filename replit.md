...
- **July 2026**: SEO readiness pass: switched site domain in meta tags/canonical from tribal18.golf to https://tribal18.com; added robots.txt (allows all, disallows /admin, points to sitemap) and sitemap.xml (6 public pages) in client/public; created the missing og-image.png (1200x630 from the app screens image); added SoftwareApplication JSON-LD structured data to index.html; new useSEO hook (client/src/lib/seo.ts) sets unique title, description, canonical, and Open Graph/Twitter tags per page (home, contact, login, create-account, privacy, terms), with noindex on admin pages; in production the server also injects the same per-route meta tags into the served index.html (server/seo.ts + server/static.ts) so crawlers and social scrapers get correct tags on first fetch without running JavaScript (verified via local production build)
